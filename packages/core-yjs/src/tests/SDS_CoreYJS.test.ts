...
} from '../sds-core-yjs.js'

//----------------------------------------------------------------------------//
//                                   Tests                                    //
//----------------------------------------------------------------------------//

describe('SDS Core YJS — Re-exports', () => {
  it('YJS-01: SDS_Error is exported and constructible', () => {
  ...
  it('YJS-02: SDS_DataStore is exported', () => {
  ...
  it('YJS-03: SDS_Entry, SDS_Item, SDS_Link are exported', () => {
    expect(SDS_Entry).toBeDefined()
    expect(SDS_Item).toBeDefined()
    expect(SDS_Link).toBeDefined()
  })

  it('YJS-04: fromScratch() produces a working SDS_DataStore', () => {
  ...
  it('YJS-05: items and links are instances of SDS_Item and SDS_Link', () => {
    const Store = SDS_DataStore.fromScratch()
  ...
  it('YJS-06: patch exchange between two independent stores works', () => {
    ...
    Item.Label  = 'hello from core-yjs'

    StoreB.applyRemotePatch(StoreA.exportPatch())
    expect(StoreB.EntryWithId(Item.Id)?.Label).toBe('hello from core-yjs')
  })
})
