...
} from '../sds-core-loro.js'

//----------------------------------------------------------------------------//
//                                   Tests                                    //
//----------------------------------------------------------------------------//

describe('SDS Core LORO — Re-exports', () => {
  it('LORO-01: SDS_Error is exported and constructible', () => {
  ...
  it('LORO-02: SDS_DataStore is exported', () => {
  ...
  it('LORO-03: SDS_Entry, SDS_Item, SDS_Link are exported', () => {
    expect(SDS_Entry).toBeDefined()
    expect(SDS_Item).toBeDefined()
    expect(SDS_Link).toBeDefined()
  })

  it('LORO-04: fromScratch() produces a working SDS_DataStore', () => {
  ...
  it('LORO-05: items and links are instances of SDS_Item and SDS_Link', () => {
    const Store = SDS_DataStore.fromScratch()
  ...
  it('LORO-06: patch exchange between two independent stores works', () => {
    ...
    Item.Label  = 'hello from core-loro'

    StoreB.applyRemotePatch(StoreA.exportPatch())
    expect(StoreB.EntryWithId(Item.Id)?.Label).toBe('hello from core-loro')
  })
})
